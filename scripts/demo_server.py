...
MSG_TTL = 10 # Dismiss messages after 10 s
...
class UserSession:
    def __init__(self,
                 ...
                 user_message: Optional[Tuple[str, str, float]] = None,
    ...
    def add_user_msg(self, msg: str, kind: str="info") -> None:
        # Currently we only show one user message at a time
        self.user_message = (msg, kind, time.time() + MSG_TTL)
    def viewmodel(self) -> dict:
        ...
        if not normal: suggestion="Wait"
            ...
            suggestion="Click on 'Run benchmark'"
            highlight="locust"
        elif db_status_ok and locust_status_ok:
            suggestion="Click on 'Open Bench UI', start the benchmark, then click on 'Open Grafana'"
            highlight="latency"
        return {
            ...
            async with state_file_lock:
                ...
                for gui_secret, session_json in data["user_sessions"].items():
                    state.user_sessions[gui_secret] = UserSession(
                        ...
                        user_message=(tuple(session_json["user_message"])
                                      if session_json["user_message"]
                                      else None),
...
async def create_database(request: Request,
    ...
    db_name = session.db
    row_count = 100000
    session.add_user_msg(f"Creating database with {row_count} rows", "info")
    await state.set_session(gui_secret, session)
    request.state.release_state_lock()
    async def create_db_in_background():
        ...
        try:
            ...
            await sql(
                ...
                "USE benchmark",
                (
                     "CALL generate_table_data("
                    f"'{db_name}',"             # database name
                     "'bench_tbl',"             # table name
                     "10,"                      # column count
                    f"{row_count},"             # row count
                     "1000,"                    # batch size
                     "1)"                       # column_info
                ))
            success = True
        ...
        async with state_lock:
            ...
            async with session.lock:
                if success:
                    session.add_user_msg(f"Database created successfully, with {row_count} rows", "ok")
                else:
...
async def run_locust(request: Request,
    ...
    session.add_user_msg(f"Starting locust with {LOCUST_WORKER_COUNT} workers", "info")
    await state.set_session(gui_secret, session)
    db_name = session.db
    async def start_locust_in_background():
        ...
        async with session.lock:
            session.status = SessionStatus.NORMAL
            session.add_user_msg(f"Locust started with {LOCUST_WORKER_COUNT} workers", "ok")
            await state.set_session(gui_secret, session)
        # Update NGINX config and reload
        await update_nginx_config()
        ...
        for gui_secret, session in state.user_sessions.items():
            async with session.lock:
                ...
                if validate_gui_secret(gui_secret):
                    if session.db:
                        grafana_access.append(gui_secret)
                    if session.locust_port_offset is not None:
                        locust_access_port.append(
                            (gui_secret, 44000 + session.locust_port_offset))
        content = [
            # Map GUI secret to validity
             'map $gui_secret $grafana_access {',
            f'    "{GUI_SECRET}" 1;',
            *[f'    "{gui_secret}" 1;'
              for gui_secret in grafana_access],
             '    default 0;',
             '}',
            ...
            f'    "{GUI_SECRET}" 8089;',
            *[f'    "{gui_secret}" {port};'
              for gui_secret, port in locust_access_port],
             '    default 0;',
             '}',
        ]
    async with state_file_lock:
